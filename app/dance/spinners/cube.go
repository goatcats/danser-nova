...
func NewCubeMover() *CubeMover {
	return &CubeMover{BaseMover: &BaseMover{}}
}
...
func (c *CubeMover) GetPositionAt(time float64) vector.Vector2f {
	...
	radY := math32.Sin(sDelta/9000*2*math32.Pi) * 3.0 / 18 * math32.Pi
	radX := math32.Sin(sDelta/5000*2*math32.Pi) * 3.0 / 18 * math32.Pi

	scale := (1.0 + math32.Sin(sDelta/4500*2*math32.Pi)*0.3) * float32(spS.Radius)

	mat := mgl32.HomogRotate3DY(radY).Mul4(mgl32.HomogRotate3DX(radX)).Mul4(mgl32.Scale3D(scale, scale, scale))

	startIndex := (int64(max(0, sDelta)) / 4) % int64(len(cubeIndices))
